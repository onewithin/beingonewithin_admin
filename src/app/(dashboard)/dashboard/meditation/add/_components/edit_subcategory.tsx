...
function EditSubCategory({ subcategory, updateSubCategory }: { subcategory: any; updateSubCategory: (item: any) => void }) {
    const [open, setOpen] = useState(false)
    const [color, setColor] = useState(subcategory.color || '#2b7272')
    ...
    const onSubmit = async (data: FormData) => {
        try {
            setLoading(true)
            const body = { name: data.title, description: data.description, color }
            const res = await fetcher(categoryApi.updateSubCategory(subcategory.id), { method: 'PATCH', data: body })
            if (res) {
                updateSubCategory(res)
                setOpen(false)
    ...
    useEffect(() => {
        if (open) {
            ...
            setColor(subcategory.color || '#2b7272')
        }
    }, [open, subcategory])
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                ...
                <DialogContent className="sm:max-w-[480px] p-0 overflow-hidden rounded-2xl">
                    <DialogHeader className="px-6 pt-6 pb-4 border-b border-gray-100">
                        <DialogTitle className="text-lg font-semibold text-gray-900 font-rubik-400">Edit Subcategory</DialogTitle>
                        <p className="text-sm text-gray-500 font-rubik-400 mt-0.5">Update the subcategory details below.</p>
                    </DialogHeader>
